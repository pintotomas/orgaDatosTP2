...
# define base model
def baseline_modelRedNueronal(cantidadDeFeature):
...
class RedNeuronalRegressorParameterSearcher(object):
    ...
    def train_and_score(self, training_set, final_features):
        ...
        for x in range(1,3):
            ...
            scores  =cross_val_score(  redKerasRegressor, actual_data, labels, cv=kfold)
...
class RamdomForestRegressorParameterSearcher(object):
    ...
    def train_and_score(self, training_set, final_features):
        ...
        for x in range(5,40):
            ...
            scores = cross_val_score(ForestRegressor,actual_data, labels, cv=20, scoring='neg_mean_squared_error', n_jobs = -1) 
            scores_actuales.append((scores.mean(),[x]))
        return scores_actuales
...
implemented_models = {"knn":KNeighborsRegressorParameterSearcher(),"decisiontree":  DecisionTreeRegressorParameterSearcher(),"rforest":RamdomForestRegressorParameterSearcher(),"red":RedNeuronalRegressorParameterSearcher()}
